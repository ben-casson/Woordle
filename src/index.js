...
const gamesPlayedCountDisplay = document.getElementById('games-played-count');
...
const Game = () => {
    ...
    const increaseGamesPlayedCount = () => {
        ...
        gamesPlayedCountDisplay.innerHTML = gamesPlayedCount;
        winRateDisplay.innerHTML = parseInt(JSON.parse(localStorage.getItem('gamesWonCount')) / gamesPlayedCount * 100);
    }
...
function updateNumberofGuesses(row) {
    let rowNumber = gameRowObjects.indexOf(row) + 1;
    let currentAmountOfGuesses = JSON.parse(localStorage.getItem(`guessCount${rowNumber}`));
    currentAmountOfGuesses++;
    localStorage.setItem(`guessCount${rowNumber}`, currentAmountOfGuesses);
}

const graphZeroBlocksArray = [document.getElementById('graph-zero-block1'),
                              document.getElementById('graph-zero-block2'),
                              document.getElementById('graph-zero-block3'),
                              document.getElementById('graph-zero-block4'),
...
let getLargestGuessNumberAmount = () => {
    let largestAmount = 0;
    for (let y = 1; y < 7; y++) {
        if (JSON.parse(localStorage.getItem(`guessCount${y}`)) > largestAmount) {
            largestAmount = JSON.parse(localStorage.getItem(`guessCount${y}`));
        }
    }
    return parseInt(largestAmount);
}

function updateGuessDistributionDisplay() {
    let graphWidth = 0;
    for (let a = 1; a < 7; a++) {
        ...
        if (currentGuessCountValue > 0) {
            ...
            graphZeroBlocksArray[a - 1].innerHTML = '0';
            graphGuessAmountArray[a - 1].style.display = 'none';
        }
    }
}
...
window.onload = () => {
    if (oldWordsArrayLS === null) {
        ...
        //set initial guess distribution
        localStorage.setItem('guessCount1', '0');
        localStorage.setItem('guessCount2', '0');
        localStorage.setItem('guessCount3', '0');
        localStorage.setItem('guessCount4', '0');
        localStorage.setItem('guessCount5', '0');
        localStorage.setItem('guessCount6', '0');
    }
    else {
        gamesPlayedCountDisplay.innerHTML = JSON.parse(localStorage.getItem('gamesPlayedCount'));
        if (JSON.parse(localStorage.getItem('gamesPlayedCount')) == '0') {
            winRateDisplay.innerHTML = '0';
        }
        else {
            winRateDisplay.innerHTML = parseInt(JSON.parse(localStorage.getItem('gamesWonCount')) / JSON.parse(localStorage.getItem('gamesPlayedCount')) * 100);
        }
        currentStreakDisplay.innerHTML = JSON.parse(localStorage.getItem('currentStreak'));
        maxStreakDisplay.innerHTML = JSON.parse(localStorage.getItem('maxStreak'));
    }
    updateGuessDistributionDisplay();
